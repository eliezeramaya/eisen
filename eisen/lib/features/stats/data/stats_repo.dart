...
class StatsRepo {
  ...
  Future<WeeklyStats> computeWeeklyStats(DateTime now) async {
    ...
    final weekEnd = DateTime(now.year, now.month, now.day).add(const Duration(days: 1));
    ...
    int replans = tasks.where((t) => t.updatedAt != null && t.createdAt != null && t.updatedAt!.difference(t.createdAt!).inHours > 24).length;
    int done = tasks.where((t) => t.completedAt != null && t.completedAt!.isAfter(weekStart) && t.completedAt!.isBefore(weekEnd)).length;
    return WeeklyStats(
