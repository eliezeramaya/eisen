...
void main() {
  test('tiles smaller than 44x44 are stacked into +N', () {
    ...
    final layout =
        usecase.execute(tasks: tasks, zoom: null, viewport: viewport);

    // Find stack tile in Q1 and ensure it groups the tiny tasks
    final stack = layout
        .where(
            (e) => e.task.quadrant == Quadrant.q1 && e.stackChildren.isNotEmpty)
        .toList();
    expect(stack.length, 1,
        reason: 'Expected a single +N tile for small tasks');
    expect(stack.first.stackChildren.length, 80);

    // Ensure no other tile in Q1 is smaller than 44x44 in pixels
    final others = layout
        .where((e) => e.task.quadrant == Quadrant.q1 && e.stackChildren.isEmpty)
        .toList();
    for (final tr in others) {
      final w = tr.rect01.width * viewport.width;
      final h = tr.rect01.height * viewport.height;
      expect(w >= 44 || h >= 44, isTrue,
          reason: 'Non-stacked tile too small in one dimension');
      expect(w * h >= 44 * 44, isTrue,
          reason: 'Non-stacked tile area too small');
    }
  });
}
