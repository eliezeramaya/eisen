...
void main() {
  group('weight() monotonicity and bounds', () {
    ...
    test('weight is monotone with respect to due date proximity', () {
      ...
      final task1DayAway = baseTask.copyWith(due: now.add(const Duration(days: 1)));
      final task3DaysAway = baseTask.copyWith(due: now.add(const Duration(days: 3)));
      final task7DaysAway = baseTask.copyWith(due: now.add(const Duration(days: 7)));
      final task30DaysAway = baseTask.copyWith(due: now.add(const Duration(days: 30)));
      ...
      expect(w1 >= w3, isTrue, reason: '1 day away should weigh >= 3 days away');
      expect(w3 >= w7, isTrue, reason: '3 days away should weigh >= 7 days away');
      expect(w7 >= w30, isTrue, reason: '7 days away should weigh >= 30 days away');
    });
    ...
    test('weight is monotone with respect to priority', () {
      ...
      expect(wHigh > wMid, isTrue, reason: 'Priority 9 should weigh > priority 5');
      expect(wMid > wLow, isTrue, reason: 'Priority 5 should weigh > priority 2');
    });
    ...
    test('urgent tasks have higher weight than non-urgent (same other params)', () {
      final urgentQ1 = baseTask.copyWith(quadrant: Quadrant.q1); // Urgent & Important
      final urgentQ3 = baseTask.copyWith(quadrant: Quadrant.q3); // Urgent & Not Important
      final notUrgentQ2 = baseTask.copyWith(quadrant: Quadrant.q2); // Not Urgent & Important
      final notUrgentQ4 = baseTask.copyWith(quadrant: Quadrant.q4); // Not Urgent & Not Important
      ...
      expect(wQ1 > wQ2, isTrue, reason: 'Q1 (urgent) should weigh > Q2 (not urgent) with same params');
      expect(wQ3 > wQ4, isTrue, reason: 'Q3 (urgent) should weigh > Q4 (not urgent) with same params');
    });
    ...
    test('weight is always finite and non-negative', () {
      ...
      final tasks = [
        ...
        baseTask.copyWith(priority: 15, minutes: 500), // Above max (should clamp)
      ...
      for (final task in tasks) {
        final w = weight(task);
        expect(w.isFinite, isTrue, reason: 'Weight must be finite for task: ${task.title}');
        expect(w.isNaN, isFalse, reason: 'Weight must not be NaN for task: ${task.title}');
        expect(w >= 0, isTrue, reason: 'Weight must be non-negative for task: ${task.title}');
      }
    });
    ...
    test('weight output is within documented range', () {
      ...
      final maxTask = Task(
        ...
        priority: 10,
        minutes: 240,
        due: now.add(const Duration(hours: 1)), // Due very soon for high boost, but not past
        tags: const [],
        notes: '',
        category: '',
        createdAt: now, // Fresh
      ...
      expect(wMin >= 2.5, isTrue, reason: 'Min weight should be ~3.78 (adjusted for freshness)');
      expect(wMin <= 6.0, isTrue, reason: 'Min weight should be ~3.78 (adjusted for freshness)');
      ...
      expect(wMax > wMin * 50, isTrue, reason: 'Max weight should be much larger than min');
    });
    ...
    test('freshness decay reduces weight for stale tasks', () {
      ...
      expect(wFresh >= wStale30, isTrue, reason: 'Fresh task should weigh >= 30-day stale');
      expect(wStale30 >= wStale180, isTrue, reason: '30-day stale should weigh >= 180-day stale');
