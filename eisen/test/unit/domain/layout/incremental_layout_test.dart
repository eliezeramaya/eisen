...
void main() {
  test('incremental recompute keeps unaffected quadrants identical', () {
    ...
    tasks = tasks
        .map((t) => t.id == 'q1_a' ? t.copyWith(minutes: 200) : t)
        .toList();
    final second =
        usecase.execute(tasks: tasks, viewport: viewport, only: Quadrant.q1);

    final q2RectAfter = second.firstWhere((e) => e.task.id == 'q2_a').rect01;

    expect(q2RectAfter, equals(q2RectBefore),
        reason: 'Unaffected quadrant layout should be reused as-is');
  });
}
